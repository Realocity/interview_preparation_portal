	...
	<div class="questionBox" id="app">
		...
		<transition :duration="{ enter: 500, leave: 300 }" enter-active-class="animated zoomIn" leave-active-class="animated zoomOut" mode="out-in">
			...
			<div class="questionContainer" v-if="questionIndex<quiz.questions.length" v-bind:key="questionIndex">
				...
				<header>
					<h1 class="title is-6">SHELL-Quiz</h1>
					<!--progress-->
...
var quiz = {
	      ...
	      questions: [
	         {
	            text: "The ‘logout’ built in command is used to?",
	            responses: [
	               { text: "shutdown the computer" },
	               { text: "logoff of the computer" },
	               { text: "logout the current user" },
	               { text: "to exit the current shell", correct: true }
	            ]
	         },
	         {
	            text: "The command ‘umask -S’?",
	            responses: [
	               { text: "prints the current mask using symbolic notation", correct: true },
	               { text: "prints the current mask using octal numbers" },
	               { text: "sets the mask to 000" },
	               { text: "sets the mask to 777" }
	            ]
	         },
	         {
	            text: "The ‘mapfile’ command?",
	            responses: [
	               { text: "reads lines of standard input and assigns each to the element of an indexed array", correct: true },
	               { text: "reads lines of standard output file" },
	               { text: "reads lines of standard error file" },
	               { text: "none of the mentioned" }
	            ]
	         },
	         {
	            text: "Which option of the kill command sends the given signal name to the specified process?",
	            responses: [
	               { text: "-l" },
	               { text: "-n" },
	               {
	         ...
	         {
	            text: "Which command removes a directory from directory stack?",
	            responses: [
	               { text: "dirs " },
	               {
	                  text: "popd", correct: true
	               },
	               { text: "pushd" },
	               { text: "rm" }
	            ]
	         },
	         {
	            ...
	            responses: [
	               { text: "sleep" },
	               { text: "suspend" , correct: true},
	               { text: "disown" },
	               { text: "break" }
	            ]
	         },
	         {
	            text: "Which command identifies the resource of a command? ",
	            responses: [
	         ...
	         {
	            text: "Which command prints the accumulated user and system times for processes run from the shell? ",
	            responses: [
	               { text: "time" },
	               { text: "times", correct: true },
	               { text: "both time and times " },
	               { text: "none of the mentioned" }
	            ]
	         },
	         {
	            ...
	            responses: [
	               ...
	               { text: "caller " },
	               { text: "there is no command present for this purpose" },
	               { text: "none of the mentioned" }
	            ]
	         },
	         {
	            text: "\Which command generates \  \ possible completions for \  string \ according to \  the and write it to standard output?",
	            responses: [
	               { text: "compgen " , correct: true },
	               { text: "complete" },
	               { text: "continue" },
	               { text: "none of the mentioned" }
	            ]
